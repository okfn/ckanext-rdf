from vocab import URIRef, Literal, BNode, Graph
from vocab import DCAT, RDF
...
def consume_all(graph):
    pkgs = []
    for (s, p, o) in graph.triples((None, RDF.type, DCAT.Dataset)):
        pkgs.append(consume(graph, s))
    return pkgs

def consume_one(graph):
    for (s, p, o) in graph.triples((None, RDF.type, DCAT.Dataset)):
        return consume(graph, s)

def consume(graph, dataset):
